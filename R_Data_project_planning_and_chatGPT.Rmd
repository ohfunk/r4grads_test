...
<br>
First let's reload the fish data. Make sure you set your working directory with setwd() like wAlso lets library tidyverse so we have all those functions as well. 
...
```{r}
# Filter for genotype and temperature
gene_counts_cleaned <- gene_counts[, grep("N2_16C|WH171_16C", names(gene_counts))]
...
```{r}
# Filter for genotype and temperature
meta_data_cleaned <- filter(meta_data, geno == 'N2' & temp == '16C' | geno =='WH171' & temp =='16C')
meta_data_cleaned
# Run DESeq2
...
```
<br>
This looks pretty good, but we can do better! Lets ask chatGPT to add some text labels for out top hits.
<br>
```{r}
library(ggrepel)
volcano_data <- as.data.frame(results(dds)[, c("log2FoldChange", "padj")])
